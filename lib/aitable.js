...
export async function getRecords(datasheetId, options = {}) {
  ...
  const response = await fetch(url, {
    ...
    cache: "no-store" // Stäng av caching i dev, använd revalidate i produktion
  });
...
/**
 * Skapar ett nytt fält i en datasheet
 * @param {string} datasheetId - ID för datasheeten
 * @param {object} fieldConfig - Fält-konfiguration
 * @param {string} fieldConfig.name - Fältnamn
 * @param {string} fieldConfig.type - Fälttyp (SingleText, Text, SingleSelect, Checkbox, URL, DateTime, Email)
 * @param {object} fieldConfig.property - Typ-specifika egenskaper
 * @returns {Promise<object>} Skapat fält
 */
export async function createField(datasheetId, fieldConfig) {
  const token = process.env.AITABLE_API_TOKEN;
  const spaceId = process.env.AITABLE_SPACE_ID;
  ...
  if (!spaceId) {
    throw new Error("AITABLE_SPACE_ID is not configured");
  }

  const url = `${AITABLE_BASE_URL}/spaces/${spaceId}/datasheets/${datasheetId}/fields`;
