...
function mapRecordToPost(record) {
  ...
  // Samma hantering för canonicalUrl
  let canonicalUrl = "";
  if (fields.canonicalUrl) {
    if (typeof fields.canonicalUrl === "string") {
      canonicalUrl = fields.canonicalUrl;
    } else if (fields.canonicalUrl.text) {
      canonicalUrl = fields.canonicalUrl.text;
    }
  }
...
export async function createPost(postData) {
  ...
  // Auto-generera SEO-fält om inte specificerade
  if (!postData.metaTitle && postData.title) {
    postData.metaTitle = postData.title;
  }

  if (!postData.metaDescription && postData.description) {
    postData.metaDescription = postData.description.slice(0, 160);
  }

  if (!postData.canonicalUrl && postData.slug) {
    postData.canonicalUrl = `https://flexra.se/insikter/${postData.slug}`;
  }
