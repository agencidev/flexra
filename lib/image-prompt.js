...
export function generateImagePrompt(article) {
  ...
  return {
    ...
    negativePrompt: "blurry, low quality, distorted, deformed, ugly, bad anatomy, text, words, letters, numbers, typography, watermark, logo text, captions, labels, titles, written content"
  };
}
...
function buildPrompt(keywords, style, title, content, brandAnalysis = {}, composition = "") {
  ...
  return `Professional blog header image: ${concepts}.${brandSection}${contextSection} Style: ${style}.${compositionSection} Swedish business context. Aspect ratio 16:9. High quality, modern aesthetic. Soft, natural lighting. IMPORTANT: Do NOT include any text, words, letters, numbers, or typography in the image. The image should be purely visual without any written content.`;
}
